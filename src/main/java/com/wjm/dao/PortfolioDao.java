...
public class PortfolioDao implements PortfolioIDao {
	...
	public String createPortfolio(int account_pk, String title, String categoryId, String subcategoryId,
			...
			MultipartFile image2, String caption2, MultipartFile image3, String caption3, String tagList, String real_path) throws IOException, FileUploadException, ParseException
	{
		...
		if(check1)
		{
			img0 = Fileupload.upload_portfolio(real_path, image1, account_pk);
			if(img0.equals("error"))
				return "이미지 업로드중 에러가 발생했습니다.";
		}
		if(check2)
		{
			img1 = Fileupload.upload_portfolio(real_path, image2, account_pk);
			if(img1.equals("error"))
				return "이미지 업로드중 에러가 발생했습니다.";
		}
		if(check3)
		{
			img2 = Fileupload.upload_portfolio(real_path, image3, account_pk);
			if(img1.equals("error"))
				return "이미지 업로드중 에러가 발생했습니다.";
		}
