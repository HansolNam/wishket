package com.wjm.models;

import java.sql.Timestamp;

public class CommentInfo {
	private int pk;
	private int project_pk;
	private int account_pk;
	private String content;
	private Timestamp reg_date;
	private String id;
	public String getId() {
		return id;
	}
	public void setId(String id) {
		this.id = id;
	}
	public String getImg() {
		return img;
	}
	public void setImg(String img) {
		this.img = img;
	}
	public CommentInfo(int pk, int project_pk, int account_pk, String content, Timestamp reg_date, String id,
			String img) {
		super();
		this.pk = pk;
		this.project_pk = project_pk;
		this.account_pk = account_pk;
		this.content = content;
		this.reg_date = reg_date;
		this.id = id;
		this.img = img;
	}
	private String img;
	
	public CommentInfo(int pk, int project_pk, int account_pk, String content, Timestamp reg_date) {
		super();
		this.pk = pk;
		this.project_pk = project_pk;
		this.account_pk = account_pk;
		this.content = content;
		this.reg_date = reg_date;
	}
	public int getPk() {
		return pk;
	}
	public void setPk(int pk) {
		this.pk = pk;
	}
	public int getProject_pk() {
		return project_pk;
	}
	public void setProject_pk(int project_pk) {
		this.project_pk = project_pk;
	}
	public int getAccount_pk() {
		return account_pk;
	}
	public void setAccount_pk(int account_pk) {
		this.account_pk = account_pk;
	}
	public String getContent() {
		return content;
	}
	public void setContent(String content) {
		this.content = content;
	}
	public Timestamp getReg_date() {
		return reg_date;
	}
	public void setReg_date(Timestamp reg_date) {
		this.reg_date = reg_date;
	}
	

}
